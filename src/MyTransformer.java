...
class MyTransformer extends BodyTransformer {
    ...
    // the method in the tracker class that should be invoked at the
    // entry of each instrumented method.
    private static SootMethod before_call;
	...
	if (tracker_class == null) {
	    ...
	    before_call = tracker_class.getMethod("void beforeCall(java.lang.String)");
	}
	...
	// the id of this method, as read from "rmethods"
	String method_id = (String) 
	    Instrumenter.id_info.get(method.toString());

	// for methods that are not reachable (not in file rmethods), 
	// there is no id and we shouldn't instrument
	...
	do {
		...
		// Here need to create a JIMPLE staticinvoke that
		// calls "beforeCall" in the tracker class. the actual
		// parameter is a String representing the id of the
		// call site (as defined in file "calls"). The id is
		// the method id + the number of the call site within
		// the method. For example, the 3rd call site in
		// method 5 has id "5_3". To create a JIMPLE String
		// constant "xyz", use StringConstant.v("xyz")
		
		// YOUR CODE HERE.
	    	sc = Jimple.v().newStaticInvokeExpr(before_call,StringConstant.v(method_id + "_" + i));
	    	units.insertBefore(Jimple.v().newInvokeStmt(sc),s);
	    	i++;
	    }
